...
import { Info, Code, ExternalLink, Heart, CircleDollarSign } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 mb-12">
          ...
          <div className="md:col-span-5 space-y-6">
            <Link href="/" className="font-black text-3xl tracking-tighter text-(--neo-ink) flex items-center gap-3 group">
              <img
                src="/logo.png"
                alt="TypeZen Logo"
                width={40}
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            <ul className="grid grid-cols-1 gap-3">
              <li>
                <Link href="/editor" className="font-bold text-(--neo-muted) hover:text-(--neo-ink) transition-colors flex items-center gap-2 text-sm">
                  <ExternalLink className="w-4 h-4" /> 进入编辑器
                </Link>
              </li>
              <li><Link href="#features" className="font-bold text-(--neo-muted) hover:text-(--neo-ink) transition-colors text-sm">功能特性</Link></li>
              <li><Link href="#pricing" className="font-bold text-(--neo-muted) hover:text-(--neo-ink) transition-colors text-sm">定价版本</Link></li>
              <li><Link href="#faq" className="font-bold text-(--neo-muted) hover:text-(--neo-ink) transition-colors text-sm">常见问题</Link></li>
            </ul>
          </div>
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <div className="bg-(--neo-surface) p-3 border-l-4 border-(--neo-pink) border-[3px] border-(--neo-ink) shadow-[3px_3px_0px_var(--neo-shadow-core)]">
              <p className="text-xs font-bold text-(--neo-muted) leading-relaxed">
                如果您觉得 TypeZen 对您有帮助，欢迎在 GitHub 上点个 Star 或通过赞赏支持我们的持续维护。
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-8 border-t-[3px] border-(--neo-ink) flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-(--neo-muted) font-bold text-xs">
              © {new Date().getFullYear()} {SITE_BRAND} (typezen.online) · Made with <Heart className="inline w-3 h-3 text-rose-500 fill-rose-500" />
            </p>
          </div>
          <div className="flex gap-6 text-xs font-bold text-(--neo-muted)">
            <Link href="/" className="hover:text-(--neo-ink) transition-colors">隐私政策</Link>
            <Link href="/" className="hover:text-(--neo-ink) transition-colors">服务条款</Link>
          </div>
        </div>
      </div>
    </footer>
