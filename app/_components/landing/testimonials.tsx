      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {hasData ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t) => (
              <div key={t.name} className="neo-panel p-8 flex flex-col h-full bg-(--neo-surface) group hover:-translate-y-2 transition-transform">
                ...
                <div className="pt-6 border-t-2 border-(--neo-ink)">
                  ...
                  <div className="text-sm font-black text-(--neo-muted) uppercase tracking-tighter mt-1">{t.role}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="neo-panel p-20 flex flex-col items-center justify-center bg-(--neo-surface) border-dashed opacity-60">
            ...
            <p className="font-bold text-(--neo-muted)">如果您也喜欢 TypeZen，欢迎联系我们分享您的使用体验</p>
          </div>
        )}
      </div>
    </section>
